...
public class MyInterceptor implements HandlerInterceptor {
    ...
    TokenUtil tokenUtil;
    @Autowired
    Protect_Login_Util protectLoginUtil;
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object object) throws Exception {
        ...
        if (timeOfUse >= this.yangToken && timeOfUse < this.oldToken) {
            Cookie cookie = new Cookie("token", this.tokenUtil.GetToken(Integer.parseInt(map.get("id")), map.get("name"), map.get("avatar")));cookie.setPath("/");cookie.setMaxAge(259200);
            Cookie cookie1 = new Cookie("id", map.get("id"));cookie1.setPath("/");cookie1.setMaxAge(259200);
            Cookie cookie2 = new Cookie("name", map.get("name"));cookie2.setPath("/");cookie2.setMaxAge(259200);
            Cookie cookie3 = new Cookie("avatar", map.get("avatar"));cookie3.setPath("/");cookie3.setMaxAge(259200);
            response.addCookie(cookie);response.addCookie(cookie1);response.addCookie(cookie2);response.addCookie(cookie3);
            protectLoginUtil.UpdateLoginRecord(Integer.valueOf(map.get("id")));
        }
        return true;
    }
}